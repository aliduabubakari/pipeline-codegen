...
- **Target IR**: normalized intermediate contract for the shared mapping path (`schemas/target_ir_v1.json`)
- **Renderer**: family-routed generation for imperative Python targets and declarative YAML targets
- **Verifier**: manifest + syntax/structure + target-semantic integrity checks

## Supported v1 Targets and Profiles

Primary imperative targets:

- `airflow@2.8` -> `pipeline.py`
- `prefect@3.x` -> `flow.py`
- `dagster@1.8` -> `definitions.py`

Secondary declarative target:
...
- Imperative targets support `template` and `llm-assisted`; declarative targets are `template`-only in v1.
- `llm-assisted` changes imperative task internals only.
- Prefect and Dagster rendering preserves IR dependency edges.
- Declarative targets use a separate projection path and semantic YAML verification.
...
- `--llm-provider` provider name for `llm-assisted` on imperative targets
...
- Target selection stays on `--target`; routing to imperative or declarative generation happens internally from the target profile.
- `kestra` is a semantic template backend in v1 of the declarative path.
- `llm-assisted` is rejected for declarative targets.
...
Regenerate the full proof matrix:

- imperative targets: `template`, `llm-assisted`
- declarative targets: `template`

```bash
python scripts/generate_suite_proofs.py
```

Run a full CLI matrix:

```bash
for fixture in ok_sequential.opos.yaml ok_parallel_like_dag.opos.yaml ok_notifier.opos.yaml; do
  for target_version in "airflow:2.8" "prefect:3.x" "dagster:1.8"; do
  ...
  out_dir="/tmp/opos_codegen/${fixture%.opos.yaml}/kestra/template"
  opos-generate \
    --input "samples/opos_codegen_inputs/${fixture}" \
    --target kestra \
    --target-version 0.18 \
    ...
    --strict \
    --json-report > /tmp/opos_codegen_report.json
  echo "OK fixture=${fixture} target=kestra@0.18 mode=template"
done
```
